...
% =========================================================================
% [PASTE THIS BLOCK HERE] SCIENTIFIC, MEMORY & SNR FIXES
% =========================================================================

% 1. LOW MEMORY FIX (Prevent Crash)
num_multG = 2;      % Force grid resolution to 1
Q_T_quant = 2;
Q_R_quant = 2;

% 2. HARDWARE OVERRIDE (Match Paper: 32 Rx Antennas)
% We must update BOTH the estimator variable (Qbar) AND the physics struct (p_ch)
Qbar_R_h = 4;
Qbar_R_v = 2;
Qbar_R = 8;        % Estimator expects 8 elements per subarray

% 3. ENVIRONMENT & PHYSICS OVERRIDE
% We use the EXACT variable names from your 'generate_channel_param_TIV.m' file
if exist('p_ch','var')
    p_ch.BW = 10e9;        % Bandwidth
    p_ch.d_TR = 1.0;       % Distance
    p_ch.dist = 1.0;
    
    % --- NAME FIXES (Critically Important) ---
    p_ch.Nsub_c = 8;       % FIXED: Use 'Nsub_c' (not K) to match your file
    
    % Force Channel to match 8 elements (4x2) to prevent mismatch
    % This overrides the 64-element default in the file
    p_ch.Mar = 4;          
    p_ch.Nar = 2; 
    % --- CRITICAL RE-CALCULATION (Prevents "Index Exceeds" Crash) ---
    % We changed subcarriers, so we must regenerate frequency arrays & K_abs
    p_ch = get_FrequencyRng_param(p_ch); 
    K_abs = get_Abs_Coef(p_ch);
end
B_sys = 10e9;
K = 8;             % Update loop variable (Must match p_ch.Nsub_c)

% 4. FORCE UPDATE (Consistency Check)
Q_R = 4;           % Number of Rx Subarrays
Q_R_tot = 32;      % Total Rx Antennas (4 * 8)
% =========================================================================
%% Main Simulation Parameters
E = 100; % Number of trials/runs/iterations
...
Tx_power_tot_dBm = -19.26:5:33.74; % Tx power (dBm)
...
fprintf('G_T = %d\n', size(Abar_T,2));
fprintf('G_R = %d\n', size(Abar_R,2));
fprintf('G   = %d\n', size(Abar_T,2)*size(Abar_R,2));
fprintf('Qbar_T = %d\n', Qbar_T);
fprintf('Qbar_R = %d\n', Qbar_R);
fprintf('Np = %d\n', Qbar_T * Qbar_R);


% [PASTE THIS AT LINE 61, RIGHT AFTER 'get_TxRxDict;']

% 1. Fix Memory Crash: Convert huge dictionaries to Single Precision (4 bytes vs 8 bytes)
