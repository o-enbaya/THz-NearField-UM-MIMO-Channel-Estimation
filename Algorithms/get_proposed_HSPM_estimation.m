...
for indx_UsedSATx = 1:Num_of_UsedSATx
    ...
    for indx_UsedSARx = 1:Num_of_UsedSARx
        ...
        lambda_reg = 1e-3; % Start here, sweep [1e-4, 1e-3, 1e-2] if needed
        
        %%%%%%%%%%%%%%%%%% OMP CS-Estimator %%%%%%%%%%%%%%%%%%
        [hest_bs_OMP, support_OMP] = OMP_SMC(y,Epsilon_OMP,Tx_pwr,Lbar,K);
        
        % --- STABLE HIGH-SNR FIX: Regularized LS Debiasing for OMP ---
        hest_bs_OMP_db = complex(zeros(size(Epsilon_OMP,2), K));
        for indx_subc = 1:K
            supp = support_OMP(:, indx_subc);
            supp = unique(supp(supp >= 1 & supp <= size(Epsilon_OMP,2)));
            if ~isempty(supp)
                A_supp = Epsilon_OMP(:, supp);
                hest_bs_OMP_db(supp, indx_subc) = (A_supp' * A_supp + lambda_reg * eye(length(supp))) \ (A_supp' * y(:, indx_subc));
            end
        end
        hest_bs_OMP = hest_bs_OMP_db;

        % --- SUPPORT RE-RANKING FOR OMP ---
        for indx_subc = 1:K
            supp = support_OMP(:, indx_subc);
            supp = supp(supp >= 1 & supp <= size(Epsilon_OMP,2));
            supp = unique(supp, 'stable');
            if ~isempty(supp)
                coeff_pow = abs(hest_bs_OMP(supp, indx_subc)).^2;
                [~, ord] = sort(coeff_pow, 'descend');
                supp_sorted = supp(ord);
                support_OMP(:, indx_subc) = 0;
                support_OMP(1:length(supp_sorted), indx_subc) = supp_sorted;
            end
        end
        ...
        % --- STABLE HIGH-SNR FIX: Regularized LS Debiasing for OMP-DR ---
        hest_bs_OMP_DR_db = complex(zeros(size(Epsilon_OMP_DR,2), K));
        for indx_subc = 1:K
            supp = support_OMP_DR(:, indx_subc);
            supp = unique(supp(supp >= 1 & supp <= size(Epsilon_OMP_DR,2)));
            if ~isempty(supp)
                A_supp = Epsilon_OMP_DR(:, supp, indx_subc);
                hest_bs_OMP_DR_db(supp, indx_subc) = (A_supp' * A_supp + lambda_reg * eye(length(supp))) \ (A_supp' * y(:, indx_subc));
            end
        end
        hest_bs_OMP_DR = hest_bs_OMP_DR_db;

        % --- SUPPORT RE-RANKING FOR OMP-DR ---
        for indx_subc = 1:K
            supp = support_OMP_DR(:, indx_subc);
            supp = supp(supp >= 1 & supp <= size(Epsilon_OMP_DR,2));
            supp = unique(supp, 'stable');
            if ~isempty(supp)
                coeff_pow = abs(hest_bs_OMP_DR(supp, indx_subc)).^2;
                [~, ord] = sort(coeff_pow, 'descend');
                supp_sorted = supp(ord);
                support_OMP_DR(:, indx_subc) = 0;
                support_OMP_DR(1:length(supp_sorted), indx_subc) = supp_sorted;
            end
        end
        ...
        % --- STABLE HIGH-SNR FIX: Regularized LS Debiasing for SOMP ---
        supp = unique(support_SOMP(:));
        supp = supp(supp >= 1 & supp <= size(Epsilon_SOMP,2));
        hest_bs_SOMP_db = complex(zeros(size(Epsilon_SOMP,2), K));
        if ~isempty(supp)
            A_supp = Epsilon_SOMP(:, supp);
            hest_bs_SOMP_db(supp, :) = (A_supp' * A_supp + lambda_reg * eye(length(supp))) \ (A_supp' * y);
        end
        hest_bs_SOMP = hest_bs_SOMP_db;

        % --- SUPPORT RE-RANKING FOR SOMP ---
        supp = support_SOMP(:);
        supp = supp(supp >= 1 & supp <= size(Epsilon_SOMP,2));
        supp = unique(supp, 'stable');
        if ~isempty(supp)
            coeff_pow = sum(abs(hest_bs_SOMP(supp, :)).^2, 2);
            [~, ord] = sort(coeff_pow, 'descend');
            supp_sorted = supp(ord);
            support_SOMP(:) = 0;
            support_SOMP(1:length(supp_sorted)) = supp_sorted;
        end
        ...
        % --- STABLE HIGH-SNR FIX: Regularized LS Debiasing for SOMP-DR ---
        supp = unique(support_SOMP_DR(:));
        supp = supp(supp >= 1 & supp <= size(Epsilon_SOMP_DR,2));
        hest_bs_SOMP_DR_db = complex(zeros(size(Epsilon_SOMP_DR,2), K));
        if ~isempty(supp)
            A_supp = Epsilon_SOMP_DR(:, supp);
            hest_bs_SOMP_DR_db(supp, :) = (A_supp' * A_supp + lambda_reg * eye(length(supp))) \ (A_supp' * y);
        end
        hest_bs_SOMP_DR = hest_bs_SOMP_DR_db;

        % --- SUPPORT RE-RANKING FOR SOMP-DR ---
        supp = support_SOMP_DR(:);
        supp = supp(supp >= 1 & supp <= size(Epsilon_SOMP_DR,2));
        supp = unique(supp, 'stable');
        if ~isempty(supp)
            coeff_pow = sum(abs(hest_bs_SOMP_DR(supp, :)).^2, 2);
            [~, ord] = sort(coeff_pow, 'descend');
            supp_sorted = supp(ord);
            support_SOMP_DR(:) = 0;
            support_SOMP_DR(1:length(supp_sorted)) = supp_sorted;
        end
